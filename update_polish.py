...
- For Relevant Files, explicitly explain what each file does based on context (e.g., "core.py - Entry point for shell completion"). Do not just list the files.
- The Code Walkthrough must trace the execution path function-by-function (e.g., `1. _main() -> 2. _main_shell_completion()`), noting the Purpose of each step. Do not restate the architecture here.
- The Learn Next section must provide a sequential reading path of the retrieved files to guide a developer through reading the implementation (e.g., `completion.py -> shell_completion.py -> _completion_shared.py`).
